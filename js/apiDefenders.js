...
/* 
    ...
    */
const getDataDefender = () => {
    ajax({
        method: 'GET',
        url: 'http://localhost:3000/defenders',
        success: (data) => {
            data.forEach(el => {
...
document.addEventListener('DOMContentLoaded', getDataDefender);
/* 
...
document.addEventListener('submit', (e) => {
    if (e.target === $crudForm) {
        ...
        if (!e.target.id.value) {
            ...
            ajax({
                method: 'PUT',
                url: `http://localhost:3000/defenders/${e.target.id.value}`,
                success: (data) => {
                    location.reload();
                },
                error: (error) => {
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.matches('.btnDelete')){
        ...
        if(conf) {
            ajax({
                method: 'DELETE',
                url: `http://localhost:3000/defenders/${e.target.dataset.id}`,
                success: (data) => {
                    location.reload();
                },
                error: (error) => {
