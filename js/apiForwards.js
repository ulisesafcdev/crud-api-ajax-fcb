...
/* 
    ...
    */
const getDataForwards = () => {
    ajax({
        method: 'GET',
        url: 'http://localhost:3000/forwards',
        success: (data) => {
            data.forEach(el => {
...
document.addEventListener('DOMContentLoaded', getDataForwards);
/* 
...
document.addEventListener('submit', (e) => {
    if (e.target === $crudForm) {
        ...
        if (!e.target.id.value) {
            ...
            ajax({
                method: 'PUT',
                url: `http://localhost:3000/forwards/${e.target.id.value}`,
                success: (data) => {
                    location.reload();
                },
                error: (error) => {
...
document.addEventListener('click', (e) => {
    ...
    if(e.target.matches('.btnDelete')){
        ...
        let conf = confirm('¿Esta seguro de eliminar el registro?');
        if(conf){
            ajax({
                method: 'DELETE',
                url: `http://localhost:3000/forwards/${e.target.dataset.id}`,
                success: (data) => {
                    location.reload();
                },
                error: (error) => {
